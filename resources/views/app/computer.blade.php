<div>
    <a href="{{ route('computer', $computer->slug) }}" class="position-relative d-flex justify-content-center align-items-center">
        <img src="{{ $computer->image->url}}" alt="" class="img-fluid border rounded">
        @if($computer->isDiscount())
            <div class="position-absolute text-light small fw-bold bg-primary bg-opacity-75 rounded end-0 top-0 px-1 m-1">
                -{{ $computer->discount_percent }}%
            </div>
        @endif
        @if($computer->isNew())
            <div class="position-absolute text-light small fw-bold bg-success bg-opacity-75 rounded start-0 top-0 px-1 m-1">
                @lang('app.new')
            </div>
        @endif
    </a>
    <div>
        ...
        <div class="small fw-bold">
            @if($computer->isDiscount())
                ...
                <span class="text-success">{{ number_format($computer->price(), 2, ".", " ") }} <small>TMT</small></span>
            @else
            ...
            <a href="{{ route('computer.busket', $computer->slug) }}">
                @if (!in_array($computer->id, $busket))
                    <div class="d-grid gap-2">
                        <button type="button" class="btn btn-warning"><i class="bi bi-plus-square-fill"></i></button>
                    </div>
